...
$result = $servicio->sucursalEspecifica($idSucursal);

$result2 = $servicio->sucursalEspecifica($idSucursal);

$result3 = $servicio->sucursalEspecifica($idSucursal);

$result4 = $servicio->sucursalEspecifica($idSucursal);
 ...
 <div  class="col-4">
     ...
     <?php
                ...
                if(mysqli_num_rows($result) >= 1){
                    while($sucursal = mysqli_fetch_assoc($result)){
                        echo " <h2>Sucursal ".$sucursal['descripcion']." </h2> <p>Telefono: ".$sucursal['telefono'].".</p>  <p>Horarios de atencion: ".$sucursal['horario'].".</p> <p>Domicilio: ".$sucursal['domicilio'].", ".$sucursal['localidad'].".</p>  <a style='color: #2279EC;' href='https://www.google.com/maps/dir/?api=1&destination=".$sucursal['latitude'].",".$sucursal['longitude']."' target='_blank'>¿Como llegar?</a> </h5>";
...
<br>
<a style="margin-top: 2px" <?php echo "href='../sucursales/sucursalesZona?idZona=".$idZona."'";  ?> > <h5 ><img style="height: 25px; width: 25px;" src="../application/resources/img/flecha.png"> Volver a Sucursales</h5></a>
    ...
    var infoWindowContent = [
        <?php if($result3->num_rows > 0){
            while($row = $result3->fetch_assoc()){ ?>
                ...
                '<h4><?php echo $row['domicilio']; ?></h4>' +
                '<h5><?php echo $row['localidad']; ?></h5>' + '</div>'],
        <?php }
        }
        ?>
    ];
